...
public class SettingsPushEventsFragment extends EvFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        includedOrganisations = new Gson().fromJson(SharedPreferencesHelper.getInstance().getString(SharedPreferencesHelper.PREFS.APP_PUSH_EVENTS_ORGS), new TypeToken<ArrayList<Integer>>(){}.getType());
    ...
    private void savePushOrganisations() {
        if (pushEventsAdapter!=null){
            SharedPreferencesHelper.getInstance().set(
                    SharedPreferencesHelper.PREFS.APP_PUSH_EVENTS_ORGS,
                    new Gson().toJson(
                            pushEventsAdapter.getActiveSources(),
    ...
    class LoadOrganisations implements Callback<ArrayList<OrganisationModel>> {
        ...
        public void success(ArrayList<OrganisationModel> organisationModels, Response response) {
            if (getView()!=null){
                pushEventsAdapter = new SettingsPushAdapter(getActivity(),organisationModels, includedOrganisations);
                rv_list.setVisibility(View.VISIBLE);
                rv_list.setAdapter(pushEventsAdapter);
            }
        }
